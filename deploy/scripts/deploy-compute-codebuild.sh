#!/bin/bash
#
# Project Aura - Deploy Compute Layer CodeBuild Project
#
# This script deploys the CodeBuild project for the Compute Layer (EKS).
# Run this ONCE per environment to set up the Compute Layer CI/CD pipeline.
#
# Usage:
#   ./deploy/scripts/deploy-compute-codebuild.sh dev
#   ./deploy/scripts/deploy-compute-codebuild.sh qa
#   ./deploy/scripts/deploy-compute-codebuild.sh prod
#
...
echo "=========================================="
echo "Project Aura - Compute Layer CodeBuild"
echo "=========================================="
...
STACK_NAME="${PROJECT_NAME}-codebuild-compute-${ENVIRONMENT}"

echo -e "${BLUE}Deploying Compute Layer CodeBuild stack: $STACK_NAME${NC}"
echo "This will create:"
echo "  - CodeBuild project for EKS cluster deployment"
echo "  - S3 bucket for compute artifacts"
echo "  - IAM roles with EKS permissions"
...
aws cloudformation $OPERATION \
    ...
    --template-body file://deploy/cloudformation/codebuild-compute.yaml \
    --parameters \
...
echo ""
echo "=========================================="
echo "Compute Layer CodeBuild Outputs"
echo "=========================================="
...
CODEBUILD_PROJECT=$(aws cloudformation describe-stacks \
    --stack-name $STACK_NAME \
    --query 'Stacks[0].Outputs[?OutputKey==`ComputeCodeBuildProjectName`].OutputValue' \
    --output text \
    --region $AWS_REGION)

ARTIFACTS_BUCKET=$(aws cloudformation describe-stacks \
    --stack-name $STACK_NAME \
    --query 'Stacks[0].Outputs[?OutputKey==`ComputeArtifactsBucketName`].OutputValue' \
    --output text \
    --region $AWS_REGION)
...
echo ""
echo "=========================================="
echo "Compute Layer CI/CD Pipeline Ready!"
echo "=========================================="
