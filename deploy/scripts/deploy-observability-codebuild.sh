#!/bin/bash
#
# Project Aura - Deploy Observability Layer CodeBuild Project
#
# This script deploys the CodeBuild project for the Observability Layer (Monitoring, Secrets, Cost Alerts).
# Run this ONCE per environment to set up the Observability Layer CI/CD pipeline.
#
# Usage:
#   ./deploy/scripts/deploy-observability-codebuild.sh dev
#   ./deploy/scripts/deploy-observability-codebuild.sh qa
#   ./deploy/scripts/deploy-observability-codebuild.sh prod
#
...
echo "=========================================="
echo "Project Aura - Observability Layer CodeBuild"
echo "=========================================="
...
STACK_NAME="${PROJECT_NAME}-codebuild-observability-${ENVIRONMENT}"

echo -e "${BLUE}Deploying Observability Layer CodeBuild stack: $STACK_NAME${NC}"
echo "This will create:"
echo "  - CodeBuild project for monitoring/secrets/alerts deployment"
echo "  - S3 bucket for observability artifacts"
echo "  - IAM roles with CloudWatch/SNS/Secrets Manager permissions"
echo "  - CloudWatch log group"
echo "  - Build failure/duration alarms"
echo ""
...
echo ""
echo "=========================================="
echo "Observability Layer CodeBuild Outputs"
echo "=========================================="
...
CODEBUILD_PROJECT=$(aws cloudformation describe-stacks \
    --stack-name $STACK_NAME \
    --query 'Stacks[0].Outputs[?OutputKey==`ObservabilityCodeBuildProjectName`].OutputValue' \
    --output text \
    --region $AWS_REGION)

ARTIFACTS_BUCKET=$(aws cloudformation describe-stacks \
    --stack-name $STACK_NAME \
    --query 'Stacks[0].Outputs[?OutputKey==`ObservabilityArtifactsBucketName`].OutputValue' \
    --output text \
    --region $AWS_REGION)
...
echo ""
echo "=========================================="
echo "Observability Layer CI/CD Pipeline Ready!"
echo "=========================================="
