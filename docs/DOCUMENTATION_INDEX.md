...
| [../src/services/vulnerability_scanner/parsing/](../src/services/vulnerability_scanner/parsing/) | **#181 Phase 1 (May 12, 2026)** -- tree-sitter AST surface for Python/JS/TS/Go/Java/Rust/C/C++: `languages.py` (extension + shebang + content sniff), `grammar_loader.py` (lazy thread-safe per-language grammar cache), `parser_pool.py` (bounded-LRU parser pool), `ast.py` (`parse_file` / `parse_source` with timeout + file-size guards). **#181 Phase 2 partial (May 12, 2026, Python + JavaScript)** adds `symbols.py` (`SymbolExtractor` protocol + dispatcher + `PythonSymbolExtractor` and `JavaScriptSymbolExtractor`; module/class/method/function emission, language-aware import extraction (relative for Python; default/named/namespace/side-effect for ES6), parameter extraction, docstrings (Python only), language-aware cyclomatic complexity (including JS `&&`/`||`/`??`), stable SHA-256 unit IDs, parent-child links). 256 tests in `tests/services/vulnerability_scanner/parsing/` |
