...
EXCEPTION_FIXTURES = [
    (
        AuraError,
        {"message": "m", "error_code": "E", "context": {"k": "v"}},
        ["message", "error_code", "context"],
    ),
    (
        ValidationError,
        {"message": "m", "field": "name", "context": {"v": 1}},
        ["message", "error_code", "context", "field"],
    ),
    (
        SchemaValidationError,
        {"message": "m", "schema_errors": [{"e": 1}]},
        ["message", "error_code", "context"],
    ),
    (
        ServiceError,
        {"message": "m", "service_name": "neptune"},
        ["message", "error_code", "context", "service_name"],
    ),
    (
        DatabaseError,
        {"message": "m", "database": "neptune", "operation": "g.V()"},
        ["message", "error_code", "operation"],
    ),
    (
        LLMError,
        {"message": "m", "model_id": "claude-3-5-sonnet"},
        ["message", "error_code", "model_id"],
    ),
    (
        IntegrationError,
        {"message": "m", "integration": "github", "status_code": 503},
        ["message", "error_code", "status_code"],
    ),
    (SecurityError, {"message": "m", "error_code": "X"}, ["message", "error_code"]),
    (AuthenticationError, {"message": "m"}, ["message", "error_code"]),
    (
        AuthorizationError,
        {"message": "m", "required_permission": "admin:write"},
        ["message", "error_code", "required_permission"],
    ),
    (
        InjectionError,
        {"message": "m", "injection_type": "prompt"},
        ["message", "error_code", "injection_type"],
    ),
    (
        ConfigurationError,
        {"message": "m", "config_key": "DB_URL"},
        ["message", "error_code", "config_key"],
    ),
    (
        AgentError,
        {"message": "m", "agent_name": "Coder"},
        ["message", "error_code", "agent_name"],
    ),
    (
        ToolExecutionError,
        {"message": "m", "tool_name": "search", "agent_name": "Coder"},
        ["message", "error_code", "tool_name"],
    ),
    (
        OrchestrationError,
        {"message": "m", "workflow_id": "wf-1"},
        ["message", "error_code", "workflow_id"],
    ),
    (
        WorkflowError,
        {"message": "m", "workflow_name": "patch_review"},
        ["message", "error_code", "workflow_name"],
    ),
    (
        ApprovalError,
        {"message": "m", "approval_id": "ap-1"},
        ["message", "error_code", "approval_id"],
    ),
    (
        SandboxError,
        {"message": "m", "sandbox_id": "sb-1"},
