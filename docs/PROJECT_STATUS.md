...
- **#181 (OPEN, multi-week pure-code):** `vulnerability_scanner/parsing` tree-sitter real implementation. **Phase 1 (AST surface) complete (May 12, 2026)** -- `parsing/{languages,grammar_loader,parser_pool,ast}.py`, 8 canonical fixtures (Python/JS/TS/Go/Java/Rust/C/C++), 6 new tree-sitter grammar pip deps, 155 tests. Phases 2 (symbol extraction) and 3 (dataflow/taint propagation) still pending; each will land in its own PR per the ticket guidance.
