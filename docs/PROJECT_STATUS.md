...
- **#181 (OPEN, multi-week pure-code):** `vulnerability_scanner/parsing` tree-sitter real implementation. **Phase 1 (AST surface, all 8 languages) complete and Phase 2 partial (symbol extraction, Python + JavaScript + TypeScript + Go + Java + Rust + C) complete (May 12, 2026).** Phase 1: 155 tests. Phase 2 Python: 55 tests. Phase 2 JavaScript: 46 tests. Phase 2 TypeScript (subclasses JS): 38 tests. Phase 2 Go (two-pass walker for receiver-bound methods): 34 tests. Phase 2 Java (class/interface/enum/record → CLASS; constructors with `return_type=None`): 37 tests. Phase 2 Rust (two-pass walker: struct/enum/trait → CLASS; impl blocks resolve to target type; `self` receivers; `use_declaration` expansion): 36 tests. Phase 2 C follow-on: `CSymbolExtractor` (function_definition → FUNCTION with return-type reconstruction across pointer_declarator wrappers so ``int *foo()`` -> ``"int *"``; struct/union/enum → CLASS including typedef-wrapped anonymous aggregates; bare prototype declarations skipped; `#include` system + local variants with delimiters stripped; pointer/array parameter unwrapping for parameter names; ``void`` placeholder param dropped), 34 tests. Phase 2 still-pending: C++ extractor in its own PR. Phase 3 (dataflow + taint) still pending.
