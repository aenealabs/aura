...
| --- | --- | --- | --- | --- |
| `image-size` (via `pptxgenjs`) | Frontend (production dep, lazy-loaded) | **At-Risk** | Two high-severity advisories with **no patch available**: CVE-2025-71329 and CVE-2025-71330, both affecting `image-size <= 2.0.2`. The lockfile resolves `1.2.1`, reached transitively through `pptxgenjs@4.0.1`. `pptxgenjs` is already the latest release and still declares `image-size: ^1.2.1`, so there is no bump that clears it -- the fix has to come from one of the two upstreams. **Exposure is narrow and was traced, not assumed.** `pptxgenjs` is lazy-imported client-side at `frontend/src/components/documentation/DiagramViewer.jsx:731` solely for PowerPoint export, and the only image it receives is `canvas.toDataURL('image/png')` (`:759`) -- bytes the application rendered itself from its own diagram. No user-uploaded or server-side image parsing reaches this dependency, and it is never exercised outside an explicit user-initiated export. | **Upstream-maintainer-gate.** No action available now. Re-evaluate on the weekly dependency-risk audit: the row clears when either `image-size` ships a patched release, or `pptxgenjs` moves to a patched range. If the exposure surface changes -- specifically if any code path starts handing `pptxgenjs` an image Aura did not generate -- this moves to **Replace-Now**, because the narrow exposure is the entire reason this is tolerable. Verify with `npm ls image-size` before assuming the chain is unchanged. | 
...
| `eslint-plugin-react` | Frontend (devDep) | **At-Risk** | Latest release `7.37.5` shipped 2025-04-03 (~13 months ago). Peer-eslint range capped at `^9.7`, despite `eslint@10.0.0` going GA on 2026-02-06 (~3 months ago) and `10.3.0` shipping 2026-05-01. No 7.37.6 / 7.38.x / 8.x exists; no visible upstream issue tracking eslint@10 peer support. This forces every `npm install` / `npm ci` in `frontend/` to use `--legacy-peer-deps`, which globally silences peer-dep conflicts and weakens our ability to catch *new* peer-dep regressions in unrelated packages. The package itself is dev-only (linting; never bundled to production), so the production blast radius is zero -- but the operational friction is real. | Two-track mitigation. **Track 1 (in effect)**: live with `--legacy-peer-deps` documented in `frontend/CLAUDE.md` as a known workaround. **Track 2 (deferred, trigger-gated)**: swap to `@eslint-react/eslint-plugin` (a.k.a. `eslint-plugin-react-x`) -- active fork by the eslint-react team, latest `5.7.5`, peer `eslint: ^10.3.0`. Originally tracked as issue #142, **closed 2026-05-13 without execution** (no upstream movement since 2025-04-03; keeping a perpetually-passive issue open is signal noise -- this register row is now the canonical tracking artifact). The swap executes when *any one* of: (a) `eslint-plugin-react` goes >18 months without a release with eslint@10 peer support (earliest fire ~2026-11-08, ~18 months after the 2025-04-03 last release), (b) a CVE lands without an upstream fix, or (c) `eslint@9.x` reaches EOL. Re-evaluation cadence: **next weekly dependency-risk audit PR** (Mondays 14:00 UTC; artifacts under `docs/security/audits/`) -- the recurring audit posts diffs against this register and will flag if `eslint-plugin-react` ships a release with eslint@10 peer support (which would move this row to Healthy and add a Replacement-Decisions-Made entry: No-Action, upstream caught up) or if the silence stretches further (which strengthens the swap case and opens a fresh execution issue at that point). |
...
Output is committed to `docs/security/audits/YYYY-WNN.md` via a weekly PR (titled `audit(deps): weekly report YYYY-WNN`) on each Monday run, labelled `audit-green` or `audit-findings`. Operator merges; no bot auto-merge. New high or critical CVEs are triaged in-PR (register edits ride the same PR as the audit file). See `docs/security/audits/README.md` for the artifact layout.
...
- The recurring audit posts diffs against this register; reviewers should update the register when triaging an audit PR rather than just silencing the alert.
...
- Runbook: `docs/runbooks/DEPENDENCY_RISK_AUDIT_RUNBOOK.md`
- Audit history: `docs/security/audits/` (one `YYYY-WNN.md` per ISO week)
- Audit-history README: `docs/security/audits/README.md`
- Tracking issue (pre-2026-05-13 audit-comment history only): `#138` -- closed after migration to file-based audits
