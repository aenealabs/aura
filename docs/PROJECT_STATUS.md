...
- **#181 (OPEN, multi-week pure-code):** `vulnerability_scanner/parsing` tree-sitter real implementation. **Phase 1 (AST surface, all 8 languages) complete and Phase 2 partial (symbol extraction, Python + JavaScript) complete (May 12, 2026).** Phase 1: `parsing/{languages,grammar_loader,parser_pool,ast}.py`, 8 canonical fixtures, 6 new tree-sitter grammar pip deps, 155 tests. Phase 2 Python: `parsing/symbols.py` with `SymbolExtractor` protocol + module-level dispatcher + `PythonSymbolExtractor` (modules/classes/methods/functions/nested definitions, decorators, async, imports including relative, parameter and return-type extraction, docstrings, cyclomatic complexity, stable SHA-256 unit IDs, parent-child links), 55 tests. Phase 2 JavaScript follow-on: `JavaScriptSymbolExtractor` in the same module (function/generator/class declarations, methods incl. static, arrow + function expressions assigned to const/let/var, ES6 imports across default/named/namespace/side-effect forms with alias-stripping, McCabe complexity counting `&&`/`||`/`??` operators, transparent walk through `export` statements), 46 tests. Phase 2 still-pending: TS, Go, Java, Rust, C, C++ extractors each in their own PRs. Phase 3 (dataflow + taint) still pending.
