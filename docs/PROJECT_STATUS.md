...
- **#181 (OPEN, multi-week pure-code):** `vulnerability_scanner/parsing` tree-sitter real implementation. **Phase 1 (AST surface, all 8 languages) complete and Phase 2 partial (symbol extraction, Python + JavaScript + TypeScript + Go + Java) complete (May 12, 2026).** Phase 1: `parsing/{languages,grammar_loader,parser_pool,ast}.py`, 8 canonical fixtures, 6 new tree-sitter grammar pip deps, 155 tests. Phase 2 Python: 55 tests. Phase 2 JavaScript: 46 tests. Phase 2 TypeScript (subclasses JS): 38 tests. Phase 2 Go (two-pass walker for receiver-bound methods): 34 tests. Phase 2 Java follow-on: `JavaSymbolExtractor` (single-pass DFS like JS/TS; class/interface/enum/record → CLASS; method_declaration + constructor_declaration → METHOD; constructors emit with `return_type=None`; nested classes; package_declaration as dotted module name; static/wildcard/regular import variants with `.*` suffix preserved; complexity covers if/while/do/for/enhanced_for/catch/switch_block_statement_group/switch_rule/ternary + `&&`/`||`), 37 tests. Phase 2 still-pending: Rust, C, C++ extractors each in their own PRs. Phase 3 (dataflow + taint) still pending.
