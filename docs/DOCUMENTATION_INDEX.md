...
| [assessments/DEV_QA_DEPLOYMENT_AUDIT_2026_05_14.md](assessments/DEV_QA_DEPLOYMENT_AUDIT_2026_05_14.md) | DEV/QA deployment capability audit -- vendor-style independent review of documented vs actual capabilities across CFN template integrity, buildspec wiring, deployment scripts, parameter overlays, CI workflow health, kill-switches, doc-to-code drift, and security posture. 8 parallel audit tracks (T1-T8) surfaced 16 findings (6 critical, 7 high, 3 medium), batched into 10 GH issues (#184-#193). 9 closed (8 fixed in-tree + 1 won't-fix), 1 partial. Findings restricted to "broken or actively misleading" (stylistic suggestions excluded by design). | May 14, 2026 |
| [DEFERRED_WORK_REGISTRY.md](DEFERRED_WORK_REGISTRY.md) | Single registry of every item paused pending an external condition: cost-gate (ADR-092 deploy phases, ADR-093 Phases 6-7), vendor-access-gate (#115 Mythos-class testing), external-endpoint-gate (#180 SNS subs), review-capacity-gate (#180 PR-review), upstream-maintainer-gate (At-Risk deps). Quarterly review cadence. Each row's "Re-engage when" cell is prefixed with the trigger type. (Renamed from `COST_GATE_DEFERRED.md` on 2026-05-13; old path redirects.) | May 13, 2026 |
| [COST_GATE_DEFERRED.md](COST_GATE_DEFERRED.md) | Redirect stub for the renamed deferred-work registry (above). Preserved so links from closed issues #138 / #142 / #180 / #181 still resolve. | May 13, 2026 |
...
| [../src/services/vulnerability_scanner/parsing/](../src/services/vulnerability_scanner/parsing/) | **#181 Phase 1 (May 12, 2026)** -- tree-sitter AST surface for Python/JS/TS/Go/Java/Rust/C/C++: `languages.py` (extension + shebang + content sniff), `grammar_loader.py` (lazy thread-safe per-language grammar cache), `parser_pool.py` (bounded-LRU parser pool), `ast.py` (`parse_file` / `parse_source` with timeout + file-size guards). **#181 Phase 2 COMPLETE (May 12, 2026, all 8 Phase-1 languages)** -- `symbols.py` houses `SymbolExtractor` protocol + dispatcher + eight registered extractors covering Python, JavaScript, TypeScript, Go, Java, Rust, C, and C++. **Phase 3.0 (pre-work):** `parsing/_parsing_helpers.py` extracts shared helpers; contract fields added. **Phase 3.1 (Python taint analyzer):** `parsing/dataflow.py` ships `DataFlowAnalyzer` Protocol + dispatcher + `TaintTaxonomy` dataclass + `PythonTaintAnalyzer` (intra-procedural + same-module return-value propagation, two-pass walker, ~110 sources/sinks/sanitizers grouped by CWE). 42 new tests. **Phase 3.2 (LLM analyzer integration):** `prompt_builder.build_analysis_prompt` enriched to surface `confidence`, `cwe_ids`, `taint_label`, `hop_count`; 5-chain cap + truncation marker; confidence rubric explained. 14 prompt-builder tests. **Phase 3.3 (JavaScript taint analyzer):** 38 tests. **Phase 3.4-3.9 (TypeScript / Go / Java / Rust / C / C++ taint analyzers): ALL SHIPPED** via parallel agents on isolated git worktrees (cherry-picked sequentially onto main). Each language has its own `<LANG>_TAXONOMY` + `<Lang>TaintAnalyzer` in `parsing/dataflow.py`. TS subclasses JS with type-assertion unwrapping. Go / Java / Rust / C++ are standalone two-pass walkers. C is single-pass with no cross-class resolution. Cumulative: 208 new dataflow tests across the six languages. **Phase 3.10 (cross-file inter-procedural, in-memory v1) SHIPPED (May 12, 2026):** new `FunctionSummary` Protocol + `CrossFileTaintContext` dataclass index per-function summaries by `CodeUnit.unit_id` with secondary indexes by short and qualified callable name; `DataFlowAnalyzer.analyze` Protocol signature gains `cross_file_context: Optional[CrossFileTaintContext]`; `extract_dataflows` dispatcher threads the context through with a `TypeError` fallback for analyzers that pre-date Phase 3.10. PythonTaintAnalyzer populates the context in pass 1 and consumes it in `_expression_taint` (cross-file source) and `_handle_call` (cross-file sink), with file-boundary verification so intra-file lookups never double-emit. New `_CONFIDENCE_CROSS_FILE = 0.50` constant (below 0.75 same-module cross-fn). 18 new tests. **Phase 3.11 (cross-file wiring for the remaining 7 analyzers) SHIPPED (May 13, 2026):** JS / TS / Go / Java / Rust / C / C++ analyzers all accept `cross_file_context` in `analyze()`, populate per-function summaries in pass 1, and consume in `_handle_call` + `_expression_taint` with file-boundary verification and `_CONFIDENCE_CROSS_FILE` floor enforced in `_emit_chain`. C++ uses `::` qualifier-separator for short-name fallback; the others use `.`. 18 new tests (4 JS, 2 TS, 3 Go, 3 Java, 2 Rust, 2 C, 2 C++). **Phase 3.12 (scan-wide assembly bridge) SHIPPED (May 13, 2026):** `parsing/scan_assembly.py` exposes `ParsedFile`, `ScanParseBundle`, and `assemble_dataflow_for_scan()`. Orchestrates the full parse-symbol-dataflow pipeline with the two-pass cross-file workflow internalized; returns chains indexed by SINK `code_unit_id` (the shape `LLMVulnerabilityAnalyzer.analyze_candidates` expects). Per-file parse / extract failures degrade gracefully (logged, not raised). `cross_file_context` exposed on the bundle so a Neptune-backed resolver can swap the in-memory implementation without changing call sites. 15 new tests. **ADR-093 Phase 1 (Test substrate) SHIPPED (May 13, 2026):** `TaintContext` Protocol added in `dataflow.py`; existing `CrossFileTaintContext` dataclass renamed `InMemoryTaintContext` with one-release back-compat alias; all 17 `Optional["CrossFileTaintContext"]` type hints widened to `Optional["TaintContext"]`. 12-fixture equivalence harness parametrized over `{in_memory, neptune_tinker}` backends. **ADR-093 Phase 5 (Offline static-scan substitute) SHIPPED (May 13, 2026):** `scripts/adr_093_taint_schema_static_scan.py` (AST walk + read/write property-set drift detection + tenant-predicate enforcement; report at `docs/assessments/ADR_093_SCHEMA_SCAN_REPORT.md`) + `scripts/adr_093_taint_query_static_scan.py` (CI gate: tenant-predicate check on all `V()` / `E()` / `add_v()` chains + hardcoded `arn:aws:` ARN check across all 4 Phase-2/3/4 modules; report at `docs/assessments/ADR_093_QUERY_SCAN_REPORT.md`). Both `--fail-on-gap` for CI integration. 31 tests (14 schema + 17 query); both modules pass clean baseline. **ADR-093 Phase 4 (Kill-switch + observability + production AWS wirings) SHIPPED (May 13, 2026):** `parsing/neptune_taint_aws_deps.py` with boto3 wrappers (KmsSignatureSigner/Verifier, DynamoDbWriteFence, DynamoDbRevocationOracle, SsmFlagReader, CloudWatchMetricEmitter); `scripts/disable_neptune_taint_resolver.py` (kill-switch via SSM) + `scripts/revoke_summary_signatures.py` (revocation table writer); `deploy/cloudformation/taint-resolver-iam.yaml` (Layer 9.4 writer role + permission boundary with explicit denies on IAM-write and audit-tampering); `docs/runbooks/NEPTUNE_TAINT_RESOLVER_RUNBOOK.md` (RTO=0/RPO=∞, 5 alarms with first-response, kill-switch + revocation procedures). 1422 tests (+32 Phase-4). **ADR-093 Phase 3 (Writer + end-of-scan flush) SHIPPED (May 13, 2026):** `SignatureSigner`/`WriteFence` Protocols + `DeterministicSigner`/`DeterministicVerifier`/`InMemoryWriteFence` test impls; `NeptuneTaintRepository.persist_summaries()` with bytecode `addV` + `addE` + transport-bound signature (Sally C-5); `TenantScopedGremlinClient` extended for `addV` writes (`property` predicate vs `has` predicate for reads); `TaintContext.flush()` method (no-op default; Neptune-backed writes the cache + marks write-fence); `InMemoryTaintContext.units_by_id` retains CodeUnit refs so flush can read `source_hash`; factory wires write-side deps; `assemble_dataflow_for_scan` accepts `commit_sha` kwarg and calls `flush()` at end-of-scan; rescan-equivalence acceptance test passes via in-process `FakeGremlin` emulator. 1390 scanner tests (+14 Phase-3). **ADR-093 Phase 2 (Resolver read path) SHIPPED (May 13, 2026):** `TenantScopedGremlinClient` (bytecode-only, rejects string-mode, requires tenant predicate in entry steps including `.E()`, 600 writes/min per-tenant quota) + `NeptuneTaintRepository` (paginated preload + lazy lookup, signature verifier + revocation oracle injection, schema-version check on read) + `NeptuneBackedTaintContext` (composes InMemoryTaintContext for preload cache) + `build_taint_context` factory (whole-scan fail-safe; degrades to in-memory on any of {flag off, tenant missing, repo factory raises, preload raises}; emits `taint.resolver.degraded_to_memory.<reason>` metric). `assemble_dataflow_for_scan` accepts `tenant_id` + `factory_deps` kwargs. 1376 scanner tests (+43 Phase-2). |
