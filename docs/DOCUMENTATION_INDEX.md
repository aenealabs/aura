...
| [../src/services/vulnerability_scanner/parsing/](../src/services/vulnerability_scanner/parsing/) | **#181 Phase 1 (May 12, 2026)** -- tree-sitter AST surface for Python/JS/TS/Go/Java/Rust/C/C++: `languages.py` (extension + shebang + content sniff), `grammar_loader.py` (lazy thread-safe per-language grammar cache), `parser_pool.py` (bounded-LRU parser pool), `ast.py` (`parse_file` / `parse_source` with timeout + file-size guards). **#181 Phase 2 partial (May 12, 2026, Python + JavaScript + TypeScript + Go + Java)** adds `symbols.py` (`SymbolExtractor` protocol + dispatcher + five extractors). TS subclasses JS for shared ES6 nodes; Python, Go, and Java are standalone. Go uses a two-pass walker for receiver-bound methods. Java handles class/interface/enum/record -> CLASS; method_declaration + constructor_declaration -> METHOD (constructors with `return_type=None`); package_declaration as dotted module name; static/wildcard imports preserve `.*` suffix. 365 tests in `tests/services/vulnerability_scanner/parsing/` |
