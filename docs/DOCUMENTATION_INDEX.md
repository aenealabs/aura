...
| [../src/services/vulnerability_scanner/parsing/](../src/services/vulnerability_scanner/parsing/) | **#181 Phase 1 (May 12, 2026)** -- tree-sitter AST surface for Python/JS/TS/Go/Java/Rust/C/C++: `languages.py` (extension + shebang + content sniff), `grammar_loader.py` (lazy thread-safe per-language grammar cache), `parser_pool.py` (bounded-LRU parser pool), `ast.py` (`parse_file` / `parse_source` with timeout + file-size guards). **#181 Phase 2 COMPLETE (May 12, 2026, all 8 Phase-1 languages)** -- `symbols.py` houses `SymbolExtractor` protocol + dispatcher + eight registered extractors covering Python, JavaScript, TypeScript, Go, Java, Rust, C, and C++. Three of the eight use two-pass walkers for receiver / impl / out-of-line method resolution (Go, Rust, C++). C++ also handles class_specifier / struct / union / enum / namespace_definition / template_declaration, with multi-level `qualified_identifier` parsing for ``foo::Bar::method`` and deduplication of inline-prototype + out-of-line-definition pairs. 476 tests in `tests/services/vulnerability_scanner/parsing/`. **Phase 3 pre-work (PR 3.0):** `parsing/_parsing_helpers.py` extracts shared byte/UTF-8/location helpers from `symbols.py`; `DataFlowChain` gains `confidence` + `cwe_ids`; `DataFlowNode` gains `start_byte` + `end_byte`; `enable_dataflow_analysis` defaults to `False` (production opts in). Phase 3.1 (Python taint analyzer) is the next PR. |
