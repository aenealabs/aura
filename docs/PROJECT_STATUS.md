...
| **Test Suite** | ~25,176+ tests as of May 13, 2026 (~25,133+ through ADR-093 Phase 1 + 43 ADR-093 Phase 2 tests: 18 TenantScopedGremlinClient + 16 NeptuneTaintRepository + 9 build_taint_context factory). The May 10 GTM audit observed 25 hard failures at maxfail in a full sweep and one order-dependent crash path now hardened (commit c8aa3df). A clean full-suite re-run with all wave-1 through wave-14 fixes is still pending; the "0 failures" claim should be re-asserted only after that run. |
