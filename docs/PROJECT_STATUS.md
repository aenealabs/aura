...
- **#181 (OPEN, multi-week pure-code):** `vulnerability_scanner/parsing` tree-sitter real implementation. **Phase 1 (AST surface, all 8 languages) and Phase 2 vertical-slice (symbol extraction, Python only) complete (May 12, 2026).** Phase 1: `parsing/{languages,grammar_loader,parser_pool,ast}.py`, 8 canonical fixtures, 6 new tree-sitter grammar pip deps, 155 tests. Phase 2 (this PR): `parsing/symbols.py` with `SymbolExtractor` protocol + module-level dispatcher + `PythonSymbolExtractor` (modules/classes/methods/functions/nested definitions, decorators, async, imports including relative, parameter and return-type extraction, docstrings, cyclomatic complexity, stable SHA-256 unit IDs, parent-child links), 55 tests covering shadowing/nested/async/decorated/path-flag edges. Phase 2 follow-ons: JS, TS, Go, Java, Rust, C, C++ extractors each in their own PRs. Phase 3 (dataflow + taint) still pending.
