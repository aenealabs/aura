#!/bin/bash
#
# Project Aura - Deploy Frontend Service CodeBuild Project
#
# This script deploys the CodeBuild project for the Frontend Service (React Dashboard).
# Run this ONCE per environment to set up the Frontend CI/CD pipeline.
#
# Usage:
#   ./deploy/scripts/deploy-frontend-codebuild.sh dev
#   ./deploy/scripts/deploy-frontend-codebuild.sh qa
#   ./deploy/scripts/deploy-frontend-codebuild.sh prod
#
...
echo "=========================================="
echo "Project Aura - Frontend Service CodeBuild"
echo "=========================================="
...
STACK_NAME="${PROJECT_NAME}-codebuild-frontend-${ENVIRONMENT}"

echo -e "${BLUE}Deploying Frontend Service CodeBuild stack: $STACK_NAME${NC}"
echo "This will create:"
echo "  - CodeBuild project for React Dashboard deployment"
echo "  - S3 bucket for frontend artifacts"
echo "  - IAM roles with ECR and EKS permissions"
echo "  - EKS access entry for kubectl"
...
aws cloudformation $OPERATION \
    ...
    --template-body file://deploy/cloudformation/codebuild-frontend.yaml \
    --parameters \
...
echo ""
echo "=========================================="
echo "Frontend Service CodeBuild Outputs"
echo "=========================================="
...
CODEBUILD_PROJECT=$(aws cloudformation describe-stacks \
    --stack-name $STACK_NAME \
    --query 'Stacks[0].Outputs[?OutputKey==`FrontendCodeBuildProjectName`].OutputValue' \
    --output text \
    --region $AWS_REGION)

ARTIFACTS_BUCKET=$(aws cloudformation describe-stacks \
    --stack-name $STACK_NAME \
    --query 'Stacks[0].Outputs[?OutputKey==`FrontendArtifactsBucketName`].OutputValue' \
    --output text \
    --region $AWS_REGION)
...
echo ""
echo "=========================================="
echo "Frontend Service CI/CD Pipeline Ready!"
echo "=========================================="
