...
- **#181 (OPEN, multi-week pure-code):** `vulnerability_scanner/parsing` tree-sitter real implementation. **Phase 1 (AST surface, all 8 languages) complete and Phase 2 partial (symbol extraction, Python + JavaScript + TypeScript + Go + Java + Rust) complete (May 12, 2026).** Phase 1: `parsing/{languages,grammar_loader,parser_pool,ast}.py`, 8 canonical fixtures, 6 new tree-sitter grammar pip deps, 155 tests. Phase 2 Python: 55 tests. Phase 2 JavaScript: 46 tests. Phase 2 TypeScript (subclasses JS): 38 tests. Phase 2 Go (two-pass walker for receiver-bound methods): 34 tests. Phase 2 Java (single-pass; class/interface/enum/record → CLASS; constructors emit with `return_type=None`): 37 tests. Phase 2 Rust follow-on: `RustSymbolExtractor` (two-pass walker like Go: struct/enum/trait → CLASS; trait method declarations emit inline; impl blocks resolve to their target type for inherent and trait impls; `self`/`&self`/`&mut self` receivers surface as `"self"`; `use_declaration` expansion across simple/grouped/wildcard/aliased forms with `::` separators; complexity covers if/while/loop/for/match_arm/try_expression + `&&`/`||`), 36 tests. Phase 2 still-pending: C, C++ extractors each in their own PRs. Phase 3 (dataflow + taint) still pending.
