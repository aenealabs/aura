#!/bin/bash
#
# Project Aura - Deploy Application Layer CodeBuild Project
#
# This script deploys the CodeBuild project for the Application Layer (Bedrock).
# Run this ONCE per environment to set up the Application Layer CI/CD pipeline.
#
# Usage:
#   ./deploy/scripts/deploy-application-codebuild.sh dev
#   ./deploy/scripts/deploy-application-codebuild.sh qa
#   ./deploy/scripts/deploy-application-codebuild.sh prod
#
...
echo "=========================================="
echo "Project Aura - Application Layer CodeBuild"
echo "=========================================="
...
STACK_NAME="${PROJECT_NAME}-codebuild-application-${ENVIRONMENT}"

echo -e "${BLUE}Deploying Application Layer CodeBuild stack: $STACK_NAME${NC}"
echo "This will create:"
echo "  - CodeBuild project for Bedrock deployment"
echo "  - S3 bucket for application artifacts"
echo "  - IAM roles with Bedrock permissions"
...
echo ""
echo "=========================================="
echo "Application Layer CodeBuild Outputs"
echo "=========================================="
...
CODEBUILD_PROJECT=$(aws cloudformation describe-stacks \
    --stack-name $STACK_NAME \
    --query 'Stacks[0].Outputs[?OutputKey==`ApplicationCodeBuildProjectName`].OutputValue' \
    --output text \
    --region $AWS_REGION)

ARTIFACTS_BUCKET=$(aws cloudformation describe-stacks \
    --stack-name $STACK_NAME \
    --query 'Stacks[0].Outputs[?OutputKey==`ApplicationArtifactsBucketName`].OutputValue' \
    --output text \
    --region $AWS_REGION)
...
echo ""
echo "=========================================="
echo "Application Layer CI/CD Pipeline Ready!"
echo "=========================================="
