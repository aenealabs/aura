...
| [../src/services/vulnerability_scanner/parsing/](../src/services/vulnerability_scanner/parsing/) | **#181 Phase 1 (May 12, 2026)** -- tree-sitter AST surface for Python/JS/TS/Go/Java/Rust/C/C++: `languages.py` (extension + shebang + content sniff), `grammar_loader.py` (lazy thread-safe per-language grammar cache), `parser_pool.py` (bounded-LRU parser pool), `ast.py` (`parse_file` / `parse_source` with timeout + file-size guards). **#181 Phase 2 partial (May 12, 2026, Python + JavaScript + TypeScript + Go + Java + Rust + C)** adds `symbols.py` (`SymbolExtractor` protocol + dispatcher + seven extractors). TS subclasses JS for shared ES6 nodes; Python, Go, Java, Rust, and C are standalone. Go and Rust use two-pass walkers. C reconstructs return types across `pointer_declarator` wrappers (so ``int *foo()`` yields ``"int *"``), emits struct/union/enum + typedef-wrapped anonymous aggregates as CLASS, skips bare prototype declarations, and strips `#include` delimiters for system and local includes alike. 435 tests in `tests/services/vulnerability_scanner/parsing/` |
