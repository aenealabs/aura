...
| **Test Suite** | ~25,088+ tests as of May 12, 2026 (~25,070+ baseline through Phase 3.9 + 18 Phase 3.10 cross-file dataflow tests; 24,950+ wave-5a baseline + ~18 wave-6 commit_msg_hook tests + 6 wave-8 dashboard-metrics endpoint tests + 2 wave-9 system-health endpoint tests + 37 wave-13 static-action-scan tests + 42 wave-12/14 ADR-092 IAM policy tests + 18 Phase 3.10 tests). The May 10 GTM audit observed 25 hard failures at maxfail in a full sweep and one order-dependent crash path now hardened (commit c8aa3df). A clean full-suite re-run with all wave-1 through wave-14 fixes is still pending; the "0 failures" claim should be re-asserted only after that run. |
