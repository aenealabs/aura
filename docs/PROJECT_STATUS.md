...
| **Test Suite** | ~25,070+ tests as of May 12, 2026 (24,950+ baseline through wave-5a + ~18 wave-6 commit_msg_hook tests + 6 wave-8 dashboard-metrics endpoint tests + 2 wave-9 system-health endpoint tests + 37 wave-13 static-action-scan tests + 42 wave-12/14 ADR-092 IAM policy tests). The May 10 GTM audit observed 25 hard failures at maxfail in a full sweep and one order-dependent crash path now hardened (commit c8aa3df). A clean full-suite re-run with all wave-1 through wave-14 fixes is still pending; the "0 failures" claim should be re-asserted only after that run. |
...
- **#181 (OPEN, multi-week pure-code):** `vulnerability_scanner/parsing` tree-sitter real implementation. **Phase 1 (AST surface, all 8 languages) complete and Phase 2 partial (symbol extraction, Python + JavaScript + TypeScript + Go) complete (May 12, 2026).** Phase 1: `parsing/{languages,grammar_loader,parser_pool,ast}.py`, 8 canonical fixtures, 6 new tree-sitter grammar pip deps, 155 tests. Phase 2 Python: `parsing/symbols.py` with `SymbolExtractor` protocol + module-level dispatcher + `PythonSymbolExtractor`, 55 tests. Phase 2 JavaScript follow-on: `JavaScriptSymbolExtractor` (function/generator/class declarations, methods incl. static, arrow + function expressions assigned to const/let/var, ES6 imports across default/named/namespace/side-effect forms with alias-stripping, McCabe complexity counting `&&`/`||`/`??`, transparent walk through `export` statements), 46 tests. Phase 2 TypeScript follow-on: `TypeScriptSymbolExtractor` (subclasses the JS extractor; adds return-type annotation extraction, `required_parameter`/`optional_parameter` handling, `interface_declaration` → CLASS with `method_signature` members → METHOD, `abstract_class_declaration` + `abstract_method_signature`, `enum_declaration` → CLASS, and TS-aware module-name extension list), 38 tests. Phase 2 Go follow-on: `GoSymbolExtractor` (two-pass walker: first pass emits struct/interface CLASS units from `type_declaration` and populates a type-name→unit_id lookup; second pass emits `function_declaration` and `method_declaration` units with receiver-type resolution so methods attach to their struct/interface as METHOD; multi-return type capture as `(string, error)`; package name from `package_clause` as module name; single + grouped imports with alias and dot-import stripping; complexity covers `if`/`for`/`switch`/`case`/`select` and `&&`/`||`), 34 tests. Phase 2 still-pending: Java, Rust, C, C++ extractors each in their own PRs. Phase 3 (dataflow + taint) still pending.
